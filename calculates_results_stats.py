...
def calculates_results_stats(results_dic):
    ...
    results_stats_dic["n_dogs_img"] = sum(d[3] for d in results_dic.values())
    results_stats_dic["n_notdogs_img"] = results_stats_dic["n_images"] - results_stats_dic["n_dogs_img"]
    results_stats_dic["n_match"] = sum(d[2] for d in results_dic.values())
    results_stats_dic["n_correct_dogs"] = sum(d[3] * d[4] for d in results_dic.values())
    results_stats_dic["n_correct_notdogs"] = sum((0 if d[3] == 1 else 1)*(0 if d[4] == 1 else 1) 
                                             for d in results_dic.values())
    results_stats_dic["n_correct_breed"] = sum(d[2] * d[3] for d in results_dic.values())
